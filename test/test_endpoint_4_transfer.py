...
"""
Test Endpoint 4: PATCH /api/employees/{employeeId}/transfer
Validacion del endpoint de transferencia de empleados
Puntaje total: 3.0 puntos
"""
...
class TestEndpoint4Transfer:
    ...
    def test_01_transfer_success(self):
        employee_id = 1
        payload = {"newDepartmentId": 2}
        response = requests.patch(f"{ENDPOINT}/{employee_id}/transfer", json=payload)
        try:
            assert response.status_code == 200, f"Expected 200, got {response.status_code}"
            data = response.json()
            assert "employee" in data, "Response debe contener campo 'employee'"
            assert data["employee"]["id"] == employee_id, "Employee id incorrecto"
            assert data["employee"]["departmentId"] == 2, "DepartmentId no actualizado"
            print("Test 1 PASSED: Transferencia exitosa")
            self.test_results.append(True)
    ...
    def test_02_transfer_conflict(self):
        employee_id = 1
        payload = {"newDepartmentId": 2}
        response = requests.patch(f"{ENDPOINT}/{employee_id}/transfer", json=payload)
        try:
            ...
            data = response.json()
            assert "message" in data, "Response debe contener campo 'message'"
            print("Test 2 PASSED: Conflicto por mismo departamento")
            self.test_results.append(True)
        except AssertionError as exc:
    ...
    def test_03_employee_not_found(self):
        employee_id = 999
        payload = {"newDepartmentId": 2}
        response = requests.patch(f"{ENDPOINT}/{employee_id}/transfer", json=payload)
        try:
            ...
            data = response.json()
            assert "message" in data, "Response debe contener campo 'message'"
            print("Test 3 PASSED: Empleado no encontrado retorna 404")
            self.test_results.append(True)
        except AssertionError as exc:
    ...
    def test_04_department_not_found(self):
        employee_id = 1
        payload = {"newDepartmentId": 999}
        response = requests.patch(f"{ENDPOINT}/{employee_id}/transfer", json=payload)
        try:
            assert response.status_code == 404, f"Expected 404, got {response.status_code}"
            data = response.json()
            assert "message" in data, "Response debe contener campo 'message'"
            print("Test 4 PASSED: Departamento no encontrado retorna 404")
            self.test_results.append(True)
        except AssertionError as exc:
    ...
    def test_05_department_inactive(self):
        employee_id = 1
        payload = {"newDepartmentId": 5}  # inactive department
        response = requests.patch(f"{ENDPOINT}/{employee_id}/transfer", json=payload)
        try:
            assert response.status_code == 404, f"Expected 404, got {response.status_code}"
            data = response.json()
            assert "message" in data, "Response debe contener campo 'message'"
            print("Test 5 PASSED: Departamento inactivo retorna 404")
            self.test_results.append(True)
        except AssertionError as exc:
    ...
    @staticmethod
    def cleanup():
        # restore employee 1 to department 1 if needed
        try:
            requests.patch(f"{ENDPOINT}/1/transfer", json={"newDepartmentId": 1})
        except Exception:
            pass
...
def run_all_tests(show_output: bool = True):
    if show_output:
        print("=" * 80)
        print("EJECUTANDO TESTS - ENDPOINT 4: PATCH /api/employees/{employeeId}/transfer")
        print("=" * 80)
        print()
    ...
    tests = [
        test_class.test_01_transfer_success,
        test_class.test_02_transfer_conflict,
        test_class.test_03_employee_not_found,
        test_class.test_04_department_not_found,
        test_class.test_05_department_inactive,
    ]
